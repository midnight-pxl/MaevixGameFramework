	...
	EMCore_EventScope EventScope)
{
	...
	if (!World)
	{
		UE_LOG(LogMaevixEvent, Warning, TEXT("EventFunctionLibrary::RouteEventToSubsystem -- no valid world context"));
		return;
	}
	...
	UE_LOG(LogMaevixEvent, Verbose, TEXT("EventFunctionLibrary::RouteEventToSubsystem -- routing event: %s (Scope: %s)"),
	...
	case EMCore_EventScope::Global:
	{
		...
		if (!GameInstance)
		{
			UE_LOG(LogMaevixEvent, Warning, TEXT("EventFunctionLibrary::RouteEventToSubsystem -- no valid game instance"));
			return;
		}
