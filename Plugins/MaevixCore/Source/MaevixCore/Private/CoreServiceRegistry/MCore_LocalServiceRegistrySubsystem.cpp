...
void UMCore_LocalServiceRegistrySubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
	UE_LOG(LogMaevixServices, Verbose, TEXT("LocalServiceRegistrySubsystem::Initialize: ready"));
}

FMCore_ServiceHandle UMCore_LocalServiceRegistrySubsystem::RegisterProvider(
	UClass* InterfaceClass, FGameplayTag Discriminator, UObject* Provider)
{
	...
	case EMCore_ServiceRegisterResult::Added:
		UE_LOG(LogMaevixServices, Verbose,
			TEXT("LocalServiceRegistrySubsystem::RegisterProvider: registered %s [%s] (id=%u)"),
			*GetNameSafe(InterfaceClass), *DiscriminatorName, NewId);
		break;

	case EMCore_ServiceRegisterResult::ReclaimedStale:
		UE_LOG(LogMaevixServices, Verbose,
			TEXT("LocalServiceRegistrySubsystem::RegisterProvider: reclaimed stale slot for %s [%s] (id=%u)"),
			*GetNameSafe(InterfaceClass), *DiscriminatorName, NewId);
		break;

	case EMCore_ServiceRegisterResult::RejectedDuplicate:
		UE_LOG(LogMaevixServices, Warning,
			TEXT("LocalServiceRegistrySubsystem::RegisterProvider: rejected %s as a duplicate for %s [%s]; a live provider is already registered."),
			*GetNameSafe(Provider), *GetNameSafe(InterfaceClass), *DiscriminatorName);
		break;

	case EMCore_ServiceRegisterResult::InvalidArguments:
		UE_LOG(LogMaevixServices, Warning,
			TEXT("LocalServiceRegistrySubsystem::RegisterProvider: invalid arguments (InterfaceClass=%s, Provider=%s)"),
...
UObject* UMCore_LocalServiceRegistrySubsystem::ResolveProvider(UClass* InterfaceClass, FGameplayTag Discriminator)
{
	return Registry.Resolve(InterfaceClass, Discriminator);
}

bool UMCore_LocalServiceRegistrySubsystem::UnregisterById(uint32 RegistrationId)
{
	return Registry.Unregister(RegistrationId);
}

void UMCore_LocalServiceRegistrySubsystem::PruneStale()
{
