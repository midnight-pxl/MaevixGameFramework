...
void UMCore_UISubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::Initialize: skipping on dedicated server"));
		return;
	}
	...
	else if (DevSettings && DevSettings->IsValidThemeIndex(DevSettings->DefaultThemeIndex))
	{
		...
		UE_LOG(LogMaevixUI, Verbose, TEXT("UISubsystem::Initialize: loaded default theme from index %d"),
	...
	UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::Initialize: initialized for LocalPlayer"));
}

void UMCore_UISubsystem::Deinitialize()
{
	UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::Deinitialize: cleaning up"));
...
void UMCore_UISubsystem::LoadWidgetClasses()
{
	...
	if (!PrimaryGameLayoutClass)
	{
		...
		UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::LoadWidgetClasses: using default PrimaryGameLayoutClass"));
	}
...
void UMCore_UISubsystem::CreatePrimaryGameLayout()
{
	...
	if (!PrimaryGameLayoutClass)
	{
		UE_LOG(LogMaevixUI, Error, TEXT("UISubsystem::CreatePrimaryGameLayout: PrimaryGameLayoutClass not set"));
		return;
	}
	...
	UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::CreatePrimaryGameLayout: PrimaryGameLayout created (ZOrder: %d)"),
...
UCommonActivatableWidgetStack* UMCore_UISubsystem::GetLayerStack(FGameplayTag LayerTag) const
{
	if (!LayerTag.IsValid())
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::GetLayerStack: invalid LayerTag"));
		return nullptr;
	}
	
	if (const TObjectPtr<UCommonActivatableWidgetStack> ThisStack = LayerStackMap.FindRef(LayerTag))
	{
		UE_LOG(LogMaevixUI, Verbose, TEXT("UISubsystem::GetLayerStack: Tag='%s', Stack=%s"),
			*LayerTag.ToString(), ThisStack ? *GetNameSafe(ThisStack.Get()) : TEXT("NULL"));
		
		return ThisStack;
	}
	
	UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::GetLayerStack: no stack found for tag '%s'"),
		*LayerTag.ToString());
	return nullptr;
}
	...
	FGameplayTag LayerTag)
{
	if (!WidgetClass)
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::PushWidgetToLayer: invalid WidgetClass"));
		return nullptr;
	}
	...
	if (!ThisStack)
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::PushWidgetToLayer: no stack for tag '%s'"),
			*LayerTag.ToString());
		return nullptr;
	}
	...
	if (LayerTag == MCore_UILayerTags::MCore_UI_Layer_Modal)
	{
		...
		if (!PlayerController)
		{
			UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::PushWidgetToLayer: no PlayerController for modal creation"));
			return nullptr;
		}
	...
	if (NewWidget)
	{
		...
		UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::PushWidgetToLayer: pushed '%s' to layer '%s'"),
			*WidgetClass->GetName(), *LayerTag.ToString());
	}
	else
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::PushWidgetToLayer: failed to add '%s' to layer '%s'"),
			*WidgetClass->GetName(), *LayerTag.ToString());
	}
...
void UMCore_UISubsystem::CloseScreen(UCommonActivatableWidget* Screen)
{
	...
	if (!bWasTracked)
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::CloseScreen: widget '%s' was not tracked"),
			*GetNameSafe(Screen));
	}
...
UMCore_GameMenuHub* UMCore_UISubsystem::OpenMenuHub()
{
	if (!MenuHubClass)
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::OpenMenuHub: MenuHubClass not loaded"));
		return nullptr;
	}
...
bool UMCore_UISubsystem::SetActiveThemeByIndex(int32 ThemeIndex)
{
	...
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::SetActiveThemeByIndex: invalid theme index %d"), ThemeIndex);
		return false;
	}
	...
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::SetActiveThemeByIndex: theme at index %d has no asset"), ThemeIndex);
		return false;
	}
	...
	if (!LoadedTheme)
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("UISubsystem::SetActiveThemeByIndex: failed to load theme at index %d"), ThemeIndex);
		return false;
	}
	...
	UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::SetActiveThemeByIndex: theme changed to '%s' (index %d)"),
...
void UMCore_UISubsystem::SetActiveTheme(UMCore_PDA_UITheme_Base* NewTheme)
{
	...
	UE_LOG(LogMaevixUI, Log, TEXT("UISubsystem::SetActiveTheme: theme changed to '%s'"),
