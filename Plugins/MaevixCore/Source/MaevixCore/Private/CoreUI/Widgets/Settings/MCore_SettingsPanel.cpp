...
#include "CoreData/Logging/LogMaevixSettings.h"
...
void UMCore_SettingsPanel::NativeOnActivated()
{
	...
	UE_LOG(LogMaevixSettings, Verbose,
	...
	if (!IsActivated())
	{
		UE_LOG(LogMaevixSettings, Verbose, TEXT("SettingsPanel::NativeOnActivated: early exit, not activated (BlockTags?)"));
		return;
	}
	...
	{
		UE_LOG(LogMaevixSettings, Verbose, TEXT("SettingsPanel::NativeOnActivated: post-destruct rebuild, rebinding child delegates + full BuildPanel"));
	...
	{
		UE_LOG(LogMaevixSettings, Verbose, TEXT("SettingsPanel::NativeOnActivated: calling BuildPanel (first activation)"));
		BuildPanel();
	}
	else
	{
...
{
	UE_LOG(LogMaevixSettings, Verbose, TEXT("SettingsPanel::NativeOnDeactivated: this=%p, TabCount=%d"),
		this,
...
{
	UE_LOG(LogMaevixSettings, VeryVerbose, TEXT("SettingsPanel::BuildPanel: rebuilding from %d existing tabs"),
		TabbedContainer_Main->GetTabCount());
	
	const UMCore_CoreSettings* CoreSettings = UMCore_CoreSettings::Get();
	if (!CoreSettings || !CoreSettings->HasValidSettingsCollections())
	{
		UE_LOG(LogMaevixSettings, Error,
	...
	for (const FGameplayTag& ParentTag : MainTabOrder)
	{
		...
		if (ParentTag.MatchesTagExact(KeyBindingTag))
		{
			...
			if (!PageWidget)
			{
				UE_LOG(LogMaevixSettings, Warning,
		...
		UE_LOG(LogMaevixSettings, VeryVerbose,
	...
	if (UMCore_UISubsystem* UISubsystem = GetOwningLocalPlayer()->GetSubsystem<UMCore_UISubsystem>())
	{
		...
		if (UMCore_ConfirmationDialog* Dialog = Cast<UMCore_ConfirmationDialog>(CAWidget))
		{
			...
			Dialog->SetDialogMessage(
				NSLOCTEXT("MaevixCore", "ResetAllMessage", "Reset all setting to defaults?"));
			Dialog->SetButtonLabels(
				NSLOCTEXT("MaevixCore", "ResetConfirm", "Reset"),
				NSLOCTEXT("MaevixCore", "ResetCancel", "Cancel"));
...
void UMCore_SettingsPanel::HandleResetCategoryClicked()
{
	...
	if (UMCore_UISubsystem* UISubsystem = GetOwningLocalPlayer()->GetSubsystem<UMCore_UISubsystem>())
	{
		...
		if (UMCore_ConfirmationDialog* Dialog = Cast<UMCore_ConfirmationDialog>(CAWidget))
		{
			...
			Dialog->SetDialogMessage(
				FText::Format(NSLOCTEXT("MaevixCore", "ResetCategoryMessage",
