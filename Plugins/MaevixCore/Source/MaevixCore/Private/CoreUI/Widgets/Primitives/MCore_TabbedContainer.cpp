...
void UMCore_TabbedContainer::NativeOnInitialized()
{
	...
	UE_LOG(LogMaevixUI, Verbose, TEXT("TabbedContainer::NativeOnInitialized: this=%p"), this);
...
{
	UE_LOG(LogMaevixUI, Verbose, TEXT("TabbedContainer::NativeDestruct: this=%p, TabCount=%d"), this, GetTabCount());
...
bool UMCore_TabbedContainer::AddTab(FName TabID, UWidget* PageWidget)
{
	if (TabID.IsNone())
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::AddTab: invalid TabID (NAME_None)"));
		return false;
	}

	if (!PageWidget)
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::AddTab: PageWidget is null for tab '%s'"),
			*TabID.ToString());
		return false;
	}
	...
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::AddTab: tab '%s' already exists, skipping duplicate"),
			*TabID.ToString());
		return false;
	}
	...
	UE_LOG(LogMaevixUI, Verbose, TEXT("TabbedContainer::AddTab: added tab '%s' (total: %d)"),
...
bool UMCore_TabbedContainer::RemoveTab(FName TabID)
{
	if (!PageWidgets.Contains(TabID))
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::RemoveTab: tab '%s' not found"),
			*TabID.ToString());
		return false;
	}
	...
	UE_LOG(LogMaevixUI, Log, TEXT("TabbedContainer::RemoveTab: removed tab '%s' (remaining: %d)"),
		*TabID.ToString(), PageWidgets.Num());
...
{
	UE_LOG(LogMaevixUI, Verbose, TEXT("TabbedContainer::ClearAllTabs: this=%p, TabCount=%d"), this, GetTabCount());
...
bool UMCore_TabbedContainer::SelectTab(FName TabID)
{
	if (!PageWidgets.Contains(TabID))
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::SelectTab: tab '%s' not found"),
			*TabID.ToString());
		return false;
	}
...
void UMCore_TabbedContainer::HandleTabSelected(FName TabNameID)
{
	...
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::HandleTabSelected: tab '%s' not found in cache"),
			*TabNameID.ToString());
		return;
	}
	...
	UE_LOG(LogMaevixUI, Verbose, TEXT("TabbedContainer::HandleTabSelected: switched to tab '%s'"),
		*TabNameID.ToString());
}
...
bool UMCore_TabbedContainer::SetTabEnabled(FName TabID, bool bEnabled)
{
	if (!PageWidgets.Contains(TabID))
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::SetTabEnabled: tab '%s' not found"),
			*TabID.ToString());
		return false;
	}
...
bool UMCore_TabbedContainer::SetTabHidden(FName TabID, bool bIsHidden)
{
	if (!PageWidgets.Contains(TabID))
	{
		UE_LOG(LogMaevixUI, Warning, TEXT("TabbedContainer::SetTabHidden: tab '%s' not found"),
			*TabID.ToString());
		return false;
	}
