...
void UMCore_GameMenuHub::RebuildTabBar()
{
    ...
    if (!LocalPlayer)
    {
        UE_LOG(LogMaevixUI, Warning, TEXT("GameMenuHub::RebuildTabBar: no owning LocalPlayer"));
        return;
    }
    ...
    if (!UISubsystem)
    {
        UE_LOG(LogMaevixUI, Warning, TEXT("GameMenuHub::RebuildTabBar: no UISubsystem found"));
        return;
    }
    ...
    if (RegisteredScreens.IsEmpty())
    {
        if (EmptyStateWidgetClass)
        {
            ...
            if (EmptyWidget)
            {
                ...
                UE_LOG(LogMaevixUI, Log, TEXT("GameMenuHub::RebuildTabBar: showing empty state (no screens registered)"));
            }
        }
        else
        {
            UE_LOG(LogMaevixUI, Warning, TEXT("GameMenuHub::RebuildTabBar: no screens registered and EmptyStateWidgetClass not set"));
        }
        return;
    }
...
bool UMCore_GameMenuHub::SetTabEnabled(FGameplayTag TabID, bool bEnabled)
{
    ...
    if (bSuccess)
    {
        UE_LOG(LogMaevixUI, Log, TEXT("GameMenuHub::SetTabEnabled: tab '%s' is %s"),
            *TabID.ToString(),
            bEnabled ? TEXT("ENABLED") : TEXT("DISABLED"));
    }
    else
    {
        UE_LOG(LogMaevixUI, Warning, TEXT("GameMenuHub::SetTabEnabled: tab '%s' not found"),
            *TabID.ToString());
    }
...
bool UMCore_GameMenuHub::SetTabHidden(FGameplayTag TabID, bool bIsHidden)
{
    ...
    if (bSuccess)
    {
        UE_LOG(LogMaevixUI, Log, TEXT("GameMenuHub::SetTabHidden: tab '%s' is %s"),
            *TabID.ToString(),
            bIsHidden ? TEXT("HIDDEN") : TEXT("VISIBLE"));
    }
    else
    {
        UE_LOG(LogMaevixUI, Warning, TEXT("GameMenuHub::SetTabHidden: tab '%s' not found"),
            *TabID.ToString());
    }
